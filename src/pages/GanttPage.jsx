...
function exportPDF(lavorazioni, projectName, viewMode = 'week') {
  ...
  valid.forEach((lav, i) => {
    ...
    if (bW > 40 && lav.descrizione) {
      ...
      barsSvg += `<text x="${bX+8}" y="${bY+bH*0.68}" font-family="-apple-system, BlinkMacSystemFont, 'Helvetica Neue', Arial, sans-serif" font-size="9" font-weight="700" letter-spacing="0.8" fill="rgba(255,255,255,0.95)" text-rendering="geometricPrecision" clip-path="url(#${cId})">${lav.descrizione.toUpperCase()}</text>`;
    }
  });
                ...
                return (
                  <div key={lav.id} style={{position:'absolute',top:rTop,left:0,right:0,height:rH,zIndex:4}}>
                    ...
                    >
                      ...
                      {/* Label */}
                      {barW>60&&(
                        <div style={{position:'absolute',left:8,top:0,height:'100%',display:'flex',alignItems:'center',fontFamily:"-apple-system, BlinkMacSystemFont, 'Helvetica Neue', Arial, sans-serif",fontSize:9,fontWeight:700,color:'rgba(255,255,255,0.95)',letterSpacing:'0.06em',textTransform:'uppercase',whiteSpace:'nowrap',pointerEvents:'none',maxWidth:barW-24,overflow:'hidden',textOverflow:'ellipsis'}}>
                          {lav.descrizione}
                        </div>
                      )}
                      {/* Date sotto la barra */}
