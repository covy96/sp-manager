...
function exportPDF(lavorazioni, projectName, viewMode = 'week') {
  ...
  valid.forEach((lav, i) => {
    ...
    if (bW > 40 && lav.descrizione) {
      ...
      barsSvg += `<text x="${bX+8}" y="${bY+bH*0.67}" font-family="-apple-system, BlinkMacSystemFont, 'Helvetica Neue', Arial, sans-serif" font-size="10.5" font-weight="500" fill="white" clip-path="url(#${cId})">${lav.descrizione}</text>`;
    }
  });
                ...
                return (
                  <div key={lav.id} style={{position:'absolute',top:rTop,left:0,right:0,height:rH,zIndex:4}}>
                    ...
                    >
                      ...
                      {/* Label */}
                      {barW>60&&(
                        <div style={{position:'absolute',left:8,top:0,height:'100%',display:'flex',alignItems:'center',fontFamily:"-apple-system, BlinkMacSystemFont, 'Helvetica Neue', Arial, sans-serif",fontSize:11,fontWeight:500,color:'white',letterSpacing:'0',whiteSpace:'nowrap',pointerEvents:'none',maxWidth:barW-24,overflow:'hidden',textOverflow:'ellipsis'}}>
                          {lav.descrizione}
                        </div>
                      )}
                      {/* Date sotto la barra */}
