...
export default function AnalisiOffertePage() {
  ...
  // Aggrega per voce — baseline dalle voci template, poi accumula dati reali
  const righeVoci = useMemo(() => {
    // Inizializza mappa con tutte le voci template (ordine conservato)
    const map = {}; // chiave normalizzata -> { nome, order, templateId, nOfferte, ... }
    for (const t of vociTemplate) {
      ...
      map[key] = {
        ...
        templateId: t.id,
        isTemplate: true,
        nOfferte: 0, totaleOfferte: 0,
        nAccettate: 0, totaleAccettate: 0,
        totaleCommesse: 0,
      };
    }
    ...
    for (const off of offFiltrate) {
      ...
      // Risali ai prezzi lordi se erano stati salvati già scontati
      ...
      for (const v of vociNorm) {
        ...
        // Se non è una voce template, la aggiungiamo come "extra" in fondo
        if (!map[key]) {
          map[key] = {
            nome,
            order: 9999,
        ...
        if (isAccettata) {
          ent.nAccettate++;
          const prezzoScontato = sconto > 0
            ? Math.round(Number(v.prezzo || 0) * (1 - sconto / 100) * 100) / 100
            : Number(v.prezzo || 0);
          ent.totaleAccettate += prezzoScontato;
          // Ripartisci importo commessa proporzionalmente
          if (importoCommessa > 0 && lordo > 0) {
            ent.totaleCommesse += importoCommessa * (Number(v.prezzo || 0) / lordo);
          }
        }
      }
    }
  ...
  }, [offerte, commessaByNumero, vociTemplate, annoFiltro]);
  ...
  const handleSort = col => {
    ...
    else { setSortCol(col); setSortAsc(col === "order" || col === "nome"); }
  };
  ...
  const thStyle = (col) => ({
    ...
    background: sortCol === col ? T.surface2 : "transparent",
  });
  const tdStyle = (right, muted) => ({
    ...mono, fontSize: 11, color: muted ? T.muted : T.ink,
    padding: "9px 12px", textAlign: right ? "right" : "left",
    borderBottom: `0.5px solid ${T.border}`,
  });
  const arrow = col => sortCol === col ? (sortAsc ? " ↑" : " ↓") : "";

  // KPI globali (su tutte le offerte filtrate per anno)
  const offFiltrate = annoFiltro === 0
    ? offerte
    : offerte.filter(o => new Date(o.data_offerta || o.created_at).getFullYear() === annoFiltro);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20, paddingBottom: 40 }}>
      ...
      {/* Avviso se non ci sono voci template configurate */}
      {vociTemplate.length === 0 && (
        <div style={{ ...mono, fontSize: 11, color: T.muted, background: T.surface, border: `1px solid ${T.border}`, padding: "14px 16px" }}>
          Nessuna voce offerta configurata. Aggiungile nelle{" "}
          <span
            onClick={() => navigate("/impostazioni/studio")}
            style={{ color: T.navy, cursor: "pointer", textDecoration: "underline" }}
          >impostazioni studio</span>.
        </div>
      )}
      ...
      <div style={{ background: T.surface, border: `1px solid ${T.border}`, overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr>
              <th style={thStyle("nome")} onClick={() => handleSort("nome")}>Voce offerta{arrow("nome")}</th>
          ...
          <tbody>
            {sorted.length === 0 && (
              <tr>
                <td colSpan={7} style={{ ...mono, fontSize: 11, color: T.muted, textAlign: "center", padding: "40px 0" }}>
                  Nessuna voce da mostrare
                </td>
              </tr>
            )}
            {sorted.map(r => {
              const hasDati = r.nOfferte > 0;
              return (
                <tr key={r.nome}>
                  <td style={tdStyle(false)}>
                    <span>{r.nome}</span>
                    {!r.isTemplate && (
                      <span style={{ ...mono, fontSize: 9, color: T.muted, marginLeft: 6, textTransform: "uppercase" }}>libera</span>
                    )}
                  </td>
                  <td style={tdStyle(true, !hasDati)}>{hasDati ? r.nOfferte : "—"}</td>
                  <td style={tdStyle(true, !hasDati)}>{hasDati ? currency(r.totaleOfferte) : "—"}</td>
                  <td style={tdStyle(true, !hasDati)}>{hasDati ? r.nAccettate : "—"}</td>
                  <td style={{
                    ...tdStyle(true),
                    color: !hasDati ? T.muted : r.tassoConversione >= 50 ? "#1a6b3c" : r.tassoConversione > 0 ? "#854d0e" : T.muted,
                  }}>
                    {hasDati ? `${r.tassoConversione}%` : "—"}
                  </td>
                  <td style={tdStyle(true, !hasDati)}>{hasDati ? currency(r.totaleAccettate) : "—"}</td>
                  <td style={{ ...tdStyle(true), fontWeight: hasDati && r.totaleCommesse > 0 ? 600 : 400, color: !hasDati || r.totaleCommesse === 0 ? T.muted : T.ink }}>
                    {hasDati && r.totaleCommesse > 0 ? currency(r.totaleCommesse) : "—"}
                  </td>
                </tr>
              );
            })}
          </tbody>
          {sorted.some(r => r.nOfferte > 0) && (
            <tfoot>
              <tr style={{ background: T.surface2 }}>
                <td style={{ ...tdStyle(false), fontWeight: 600, fontSize: 10, color: T.muted, textTransform: "uppercase" }}>Totale</td>
      ...
      <div style={{ ...mono, fontSize: 10, color: T.muted }}>
        * "Tot. commesse" è il valore della commessa generata, ripartito proporzionalmente tra le voci attive dell'offerta.
      </div>
    </div>
  );
}
