...
export default function TeamPage() {
  ...
  const statSt = { background: T.bg, border: `0.5px solid ${T.border}`, padding: '12px 14px' };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {cards.length === 0 ? (
        <div style={{ background: T.surface, border: `0.5px solid ${T.border}`, padding: '48px 0', textAlign: 'center', fontFamily: "'IBM Plex Mono', monospace", fontSize: 11, color: T.muted }}>Nessun membro presente.</div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10 }}>
          {cards.map(({ member: m, stats: s, progress }) => (
            <div key={m.id} style={{ background: T.surface, border: `0.5px solid ${T.border}`, padding: '20px 22px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 18 }}>
                <div style={{ width: 44, height: 44, borderRadius: '50%', background: m.color || avatarColor(m.user_name || m.user_email || ""), flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 600, color: T.surface }}>
