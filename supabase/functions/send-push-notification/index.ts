...
serve(async (req) => {
  ...
  try {
    ...
    if (body.web_push_subscription) {
      ...
      return new Response(JSON.stringify({ success: true, via: "webpush" }), {
    ...
    if (!fcm_token) {
      return new Response(JSON.stringify({ error: "No FCM token or Web Push subscription" }), { status: 400 });
    }
    ...
    return new Response(JSON.stringify({ success: true, via: "fcm", fcmData }), {
      headers: { "Content-Type": "application/json" },
    });
